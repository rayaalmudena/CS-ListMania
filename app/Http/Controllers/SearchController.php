...
class SearchController extends Controller
{
    ...
    { 
	    ...
	    $user = DB::select('SELECT DISTINCT api_id, type, image, title FROM caches WHERE LOWER(title) like LOWER("%'.$search.'%") group by api_id, type, ,title, image  ORDER BY title DESC ');
	    if (count ( $user ) > 0)
